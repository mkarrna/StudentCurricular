...
if ($info) {
    ...
    if(mysqli_num_rows($result2)>0){
        ...
        $query = "INSERT INTO addstud(rollno,name,gender,dob,address,pn,sclass,course,section) VALUES ('$rollno','$name', '$gender', '$dob', '$address', '$pn', '$sc', '$co', '$sec')";
    ...
    if ($btn_name == 'Update') {
        $rollno    = $info->rollno;
        $query = "UPDATE addstud SET name = '$name', gender = '$gender', dob = '$dob', address = '$address', pn = '$pn', sclass = '$sclass', course = '$course', section = '$section' WHERE rollno = '$rollno'";
        if (mysqli_query($conn, $query)) {
            echo 'Data Updated Successfully...';
        } else {
            echo 'Allready existed';
        }
    }
}
?>